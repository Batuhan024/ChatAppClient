...
export class HomeComponent {
  ...
  constructor(
    ...
    this.hub = new signalR.HubConnectionBuilder().withUrl("https://localhost:7089/chat-hub").build();
    this.hub.start().then(()=>{
  ...
  changeUser(user: UserModel){
    ...
    this.http.get(`https://localhost:7089/api/Chats/GetChats?userId=${this.user.id}&toUserId=${this.selectedUserId}`).subscribe((res:any)=>{
      this.chats = res;
    });
  }
  ...
  sendMessage(){
    ...
    this.http.post<ChatModel>("https://localhost:7089/api/Chat/SendMessage",data).subscribe(
      (res)=> {
        this.chats.push(res);
        this.message = "";
    });
